function varargout = pdepe(m,pde,ode,ic,bc,xmesh,t,options,varargin)
...
y0 = zeros(length(temp),nx);
y0(:,1) = temp;
for j = 2:nx
  y0(:,j) = feval(ic,xmesh(j),varargin{:});
end
[node ind] = size(ode(xmesh,t(1),y0)); %应该是有几个方程
npde = length(temp)-node; %应该是有几个方程
...
% [U1,Ux1] = pdentrp(singular,m,xmesh(1),y0(1:npde,1),xmesh(2),y0(1:npde,2),xi(1));
    [U1,Ux1] = pdentrp(singular,m,xmesh(1),y0(1:npde,1),xmesh(2),y0(1:npde,2),xi(1));
    U=zeros(npde+node,1);
    U(1:npde) = U1(:,1);
    U(npde+1:npde+node) = y0(npde+1:npde+node,1);
%     [cL,fL,sL] = feval(pde,xi(1),tnow,U,Ux1,varargin{:});
[c,f,s] = feval(pde,xi(1),t(1),U(:,1),Ux1,varargin{:});
if any([size(c,1),size(f,1),size(s,1)]~=npde)
  error(message('MATLAB:pdepe:UnexpectedOutputPDEFUN',sprintf('%d',npde)))
end
[pL,qL,pR,qR] = feval(bc,xmesh(1),y0(1:npde,1),xmesh(nx),y0(1:npde,nx),t(1),varargin{:});
if any([size(pL,1),size(qL,1),size(pR,1),size(qR,1)]~=npde)
  error(message('MATLAB:pdepe:UnexpectedOutputBCFUN',sprintf('%d',npde)))
end

D = ones(npde+node,nx);%npde应该指的是有几个方程
D( c == 0, 2:nx-1) = 0;
if ~singular
  D( qL == 0, 1) = 0;
end

% D( qR == 0, nx) = 0;    %这个一句把它取消掉了

M  = spdiags( D(:), 0, (npde+node)*nx, (npde+node)*nx);%通过将Bin的列放在d指定的对角线上，创建一个m × n的稀疏矩阵S。

% Construct block-diagonal pattern of Jacobian matrix
S = kron( spdiags(ones(nx,3),-1:1,nx,nx), ones(npde+node,npde+node));
...
%   ODE15S can solve problems M(t,y)*y' = f(t,y) with mass matrix M(t,y)
  else
...
sol = zeros(nt,nx,npde+node);
for i = 1:npde+node
  sol(:,:,i) = y(:,i:npde+node:end);
end
varargout{1} = sol;
if hasEvents % [sol,t,sole,te,ie] = pdepe(...)
  varargout{2} = t(:);
  sole = zeros(length(te),nx,npde+node);
  for i = 1:npde+node
    sole(:,:,i) = ye(:,i:npde+node:end);
  end
  varargout{3} = sole;
  varargout{4} = te(:);
  varargout{5} = ie(:);
...
%---------------------------------------------------------------------------
  ...
  %   for the ODE system.
  global inter1 b t5 b00 c00  fin1 fin2 L2 tumor_mean_min
...
%       [cR,fR,sR] = feval(pde,xi(ii),tnow,U,Ux1,varargin{:});        
    ...
    up(npde+1:npde+node,:)=ode(xmesh,tnow,u);  
%     for jj = 1:nx         
...
if tnow>t5(c00)+2*inter1
    if   sum(u(1,:))/(L2+1)>fin1 
        if b(b00)==0
            t5(c00+1)=tnow;
            t5(c00+2)=tnow+inter1;
            b00=b00+1;
            c00=c00+2;
        end
    elseif sum(u(1,:))/(L2+1)<fin2
        if b(b00)>0
            t5(c00+1)=tnow;
            t5(c00+2)=tnow+inter1;
            b00=b00+1;
            c00=c00+2;
%             tumor_mean_min=sum(u(1,:))/(L2+1);
        end
    else
    end
end
% if sum(u(1,:))/(L2+1)<tumor_mean_min
%     tumor_mean_min=sum(u(1,:))/(L2+1);
% elseif sum(u(1,:))/(L2+1)>=tumor_mean_min+0.05
%     fin1=sum(u(1,:))/(L2+1);
% end
