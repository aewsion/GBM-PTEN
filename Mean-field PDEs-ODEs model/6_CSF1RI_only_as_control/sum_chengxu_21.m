...
load('D:\Matlab代码存放\2018胶质瘤\总程序\canshu113-24hours.mat','p');
Ptest1=p(1,1:22);
TimeLength=200;
TimeLength1=24;         %总时间
% x_gap=1;                %模拟时候坐标轴的间隔
L=100;                    %模拟时候坐标轴的总长度
x = 0:x_0*0.1:x_0*L;            %肿瘤生长的空间，一维的
m=3;%这个是第三种情况下，阶梯函数的最高值，代表喂药一段时间的量为m，一段时间的量为0
delt_t=1/91.5926;      %模拟时候的时间间隔
%delt_t是一天，因为3600*24/t_s= 0.0108=1/92.5926
t0=0:delt_t*24:delt_t*24*100;
...
a2=a+1;
if b~=0
    value_ERK=ERK(1,a2)+b/x_0*(ERK(1,a2+1)-ERK(1,a2));
    value_AKT=AKT(1,a2)+b/x_0*(AKT(1,c+1)-AKT(1,a2));
else
    value_ERK=ERK(1,a2);
    value_AKT=AKT(1,a2);
end
 
% f = [1,0,0,0,-A_T_EGF*u(1),-A_T_IGF1*u(1);%A_T是新的A1了
...
%      R3=k_M*u(3)-u(2).*(a_CSF1*u(4)./(K21+K22*drug+u(4))+a_A*IL4/(K3+IL4));
    ...
    R7=V1*u(5)/(K11+u(5))/(1+u(8)/K12)/(1+EGFR_I/K13)*(1-u(7))-d1*u(7);
    R8=a*(1+V21*u(7)^n/(K21^n+u(7)^n))*(1+V22*u(3)/(K22+u(9)))/(1+u(10)/K23)*(1-u(8))-d2*u(8);
    R9=V3*u(6)/(K31+u(6))/(1+u(8)/K32)/(1+IGF1R_I/K33)*(1-u(9))-d3*u(9);
    R10=V4*u(7)/(K41+u(7))*u(9)/(K42+u(9))*(1-u(10))-d4*u(10);
% 新的
...
%     EGFR_0;ERK_0;IGF1R_0;AKT_0]Q(ii,1,2); 
% u0 = [0.5.*(1+0.1.*rand(1,length(x))); x/100.*(1+0.2.*randn(1,length(x)))+2; x/100.*(1+0.2.*randn(1,length(x)))+2;0.2.*(1+0*0.01.*rand(1,length(x)));0.85.*(1+0*0.01.*rand(1,length(x)));0]; 
