...
group=1;
% NVAR =21;%model 1            % Number of variables, according to the original setting of Para0
NVAR =2;%model 2            % Number of variables, according to the original setting of Para0
% NVAR =20;%model 3            % Number of variables, according to the original setting of Para0
% NVAR =19;%model 4            % Number of variables, according to the original setting of Para0
p=ones(group,NVAR+1);
for q=1:1:group
NIND = 5;           % Number of individuals per subpopulations
MAXGEN = 3;        % maximum Number of generations
GGAP = 0.80;         % Generation gap, how many new individuals are created
...
shu_max=5;          %Maximum search range
% Build field descriptor
% FieldD = [rep(PRECI,[1, NVAR]); [shu_min*ones(1,20) 1;shu_max*ones(1,10) 1*ones(1,8) 10*ones(1,2) 11]
%               rep([1; 0; 1 ;1], [1, NVAR])];%model 1
FieldD = [rep(PRECI,[1, NVAR]); [shu_min*ones(1,2) ;shu_max*ones(1,1) 0.1]
              rep([1; 0; 1 ;1], [1, NVAR])];%model 2
...
ObjV = Objfun2(rvChrom);  %  %  %%% Objective function % Evaluate initial population
for i=1:1:MAXGEN
    gen=i  %为了在命令框看当前迭代次数
    ...
    % Evaluate offspring, call objective function
       ObjVSel = Objfun2(bs2rv(SelCh,FieldD));
...
% for j=1:1:500
[value ind]=min(ObjV);
dmax_best=chrom_value(ind,:)         %把最优个体找出来
    FieldD(2,1)=max(dmax_best(1)-0.5,0.000000001);
    FieldD(3,1)=min(dmax_best(1)+0.5,0.2);
    FieldD(2,2)=max(dmax_best(2)-0.05,0.000000001);
    FieldD(3,2)=min(dmax_best(2)+0.05,0.1);
%     FieldD(2,3)=max(dmax_best(3)-5,2);
%     FieldD(3,3)=min(dmax_best(3)+5,21);
%     FieldD(2,4)=max(dmax_best(3)-5,2);
%     FieldD(3,4)=min(dmax_best(3)+5,21);
...
chrom_new=(dmax_best-b_min)./(b_max-b_min);
...
%  set(gca,'FontWeight','Bold','FontSize',12);
%  legend('Model','Experiments');
save('ranseti_10_0.2.mat','chrom_value','value','ind','dmax_best','FieldD')
aaaaaa=0;
 Conditions_ODES_2;
 tick={'Control group 1','Gefitinib','IGF1','IGF1+Gefitinib','EGF','EGF+Gefitinib','Control group 2','Gefitinib','5 times Gefitinib','OSI-906','5 times OSI-906'};
