...
%   SXINT = DEVAL(SOL,XINT) evaluates the solution of a differential
%   equation problem at all the entries of the vector XINT. SOL is a
%   structure returned by an initial value problem solver (ODE23, ODE45,
%   ODE78, ODE89, ODE113, ODE15S, ODE23S, ODE23T, ODE23TB, ODE15I), a
%   boundary value problem solver (BVP4C, BVP5C), or a delay differential
%   equations solver (DDE23, DDESD). The elements of XINT must be in the
%   interval [SOL.x(1) SOL.x(end)]. For each I, SXINT(:,I) is the solution
%   corresponding to XINT(I).
%
...
%            DDE23, DDESD, DDENSD, BVP4C, BVP5C, ODE78, ODE89.
...
if anynan(tint) || (tdir*(tint(1) - t(1)) < 0) || (tdir*(tint(end) - t(end)) > 0)
