...
w=pi/(TimeLength*delt_t/20); %sin函数
...
% U=[u1;u2;u3;u4];
save('D:\Matlab代码存放\2018胶质瘤\总程序\U的值','U');
load('D:\Matlab代码存放\2018胶质瘤\总程序\U的值','U');
load('D:\Matlab代码存放\2018胶质瘤\总程序\Q的值','Q');
F=zeros(4,49);
for i=1:1:4
    for j=1:1:49
        F(i,j)=Q(1,j,i)-U(j+(i-1)*49,1);
    end
end
...
%    set(gca,'ytick',0:delt_t*yhinter1:delt_t*TimeLength); 
%    set(gca,'yticklabel',0:yhinter1:TimeLength1);
   set(gca,'ytick',0:1:48); 
   set(gca,'yticklabel',0:1:48);
   xlabel('Position','FontWeight','Bold','FontSize',18);ylabel('Time (Days)','FontWeight','Bold','FontSize',18);
   set(gca,'FontWeight','Bold','FontSize',12);
