...
global A_T_EGF A_T_IGF1 A_M1 A_M2 epsilon r_T a_ERK K5 a_AKT K6 d_T k_d k_M a_CSF1 K210 K220 a_A K3 B_1 B_3 b_EGF
...
sol = pdepe2(m0,@pdex4pde,@pdex4ic,@pdex4bc,x,t,options);
u1 = sol(:,:,1);
u2 = sol(:,:,2);
u3 = sol(:,:,3);
u4 = sol(:,:,4);
...
% %           drugt3=Drug_22(j1,i1);
% %           drugt4=Drug_23(j1,i1);
...
% IGF1_0=zeros(1,1001);
% y0=zeros(4,1);
% EGFR=ones(1,1001)*0.906171095;
% ERK=ones(1,1001)*0.496913113;
% IGF1R=ones(1,1001)*0.027626518;
% AKT=ones(1,1001)*0.174679186;
% C_T_0=0.5.*(1+0.1.*rand(1,length(x)));
% C_M1_0=x/100.*(1+0.2.*randn(1,length(x)))+2;
% C_M2_0=x/100.*(1+0.2.*randn(1,length(x)))+2;
% CSF1_0=0.2.*(1+0*0.01.*rand(1,length(x)));
% EGF_0=0.85.*(1+0*0.01.*rand(1,length(x)));
% IGF1_0=zeros(1,length(x));
% EGFR_I=0;
% IGF1R_I=0;
% C_T_1=zeros(101,1001);
% C_M1_1=zeros(101,1001);
% C_M2_1=zeros(101,1001);
% CSF1_1=zeros(101,1001);
% EGF_1=zeros(101,1001);
% IGF1_1=zeros(101,1001);
% C_T_1(1,:)=C_T_0; 
% C_M1_1(1,:)=C_M1_0;
% C_M2_1(1,:)=C_M2_0;
% CSF1_1(1,:)=CSF1_0;
...
%          end
%            delt_t=1/91.5926/24;
%            t=0:delt_t:delt_t*TimeLength1;
%            options=odeset('reltol',1e-3);%精度函数不能少
%            % options=odeset('reltol',1);%精度函数不能少
%            m0 = 2;%用matlab解需要设置的参数
%            sol = pdepe(m0,@pdex4pde,@pdex4ic,@pdex4bc,x,t,options);
%            C_T_0 = sol(TimeLength1+1,:,1);
%            C_M1_0 = sol(TimeLength1+1,:,2);
%            C_M2_0 = sol(TimeLength1+1,:,3);
%            CSF1_0 = sol(TimeLength1+1,:,4);
%            EGF_0 = sol(TimeLength1+1,:,5);
%            IGF1_0 = sol(TimeLength1+1,:,6);
%       end
%       C_T_1(t1+1,:)=C_T_0; 
%       C_M1_1(t1+1,:)=C_M1_0;
%       C_M2_1(t1+1,:)=C_M2_0;
...
%       IGF1_1(t1+1,:)=IGF1_0;
%   end
%   save('D:\Matlab代码存放\2018胶质瘤\总程序\第三次值','C_T_1','C_M1_1','C_M2_1','CSF1_1','EGF_1','IGF1_1');
%   load('D:\Matlab代码存放\2018胶质瘤\总程序\第三次值','C_T_1','C_M1_1','C_M2_1','CSF1_1','EGF_1','IGF1_1');
...
%    set(gca,'ytick',0:delt_t*24*20:delt_t*24*100); set(gca,'yticklabel',0:20:100);
%    set(gca,'xtick',0:x_0*200:x_0*1000); set(gca,'xticklabel',0:20:100);
...
colorbar('ylim',[0.2,0.7],'ytick',(0.2:0.1:0.7));
% colorbar('Ticks',0:0.1:1);
% colorbar;
shading interp;
   set(gca,'ytick',0:delt_t*24*40:delt_t*24*200); set(gca,'yticklabel',0:40:200);
   set(gca,'xtick',0:x_0*20:x_0*100); set(gca,'xticklabel',0:20:100);
   xlabel('Position','FontWeight','Bold','FontSize',18);ylabel('Time (Days)','FontWeight','Bold','FontSize',18);
   set(gca,'FontWeight','Bold','FontSize',12);
...
shading interp;
  ...
  figure,
hold on
%    plot(0:delt_t:delt_t*24*TimeLength,C_T_mean,'Color',[0 0 1],'m-','LineWidth',3);
   plot(0:delt_t:delt_t*24*TimeLength,C_T_mean,'b-','LineWidth',3);
   set(gca,'xtick',(0:delt_t*24*50:delt_t*24*TimeLength));
   set(gca,'xticklabel',0:50:TimeLength);
   set(gca,'ytick',(0.25:0.05:0.7));
   set(gca,'yticklabel',0.25:0.05:0.7)
   set(gca,'xlim',[0 delt_t*24*TimeLength]);
...
plot(1:TimeLength,Survival_Rate);
...
% if t>=delt_t*24*40
%      t1=t-delt_t*24*40;
%      [drug,buyao] = picture(x1,t1); %边界条件一药物的值    
%      IL4=IL4_1(x1,t1);
% else
%     drug=0;
%     IL4=0; 
% end
     [drug,buyao] = picture(x1,t); %边界条件一药物的值    
     IL4=IL4_1(x1,t);
%      [buyao,drug] = picture(x1,t); %边界条件二的时候药物的值
%      IL4=IL4_2(x1,t);
%      drug=Drug_34(x1,t);%边界条件三的时候药物的值 
%      IL4=IL4_3(x1,t);
...
%调试
%      R1=r_T*(1+a_r*u(5)./(K1+u(5))+a_r*u(6)./(K4+u(6)));  % growth rate
     R1=r_T*(1+a_ERK*u(8)/(K5+u(8))+a_AKT*u(10)/(K6+u(10)));
     D1=d_T*(1+k_d*u(2));  % death rate    
%      R3=k_M*u(3)-u(2).*(a_CSF1*u(4)./(K21+K22*drug+u(4))+a_A*IL4/(K3+IL4));
     R3=k_M*u(3)-u(2)*(a_CSF1*u(4)/(K210+K220*drug+u(4))+a_A*IL4/(K3+IL4));
%      bs=u(3)*IL4-u(6);
%     if bs<0
%         bs=0;
%     end
...
%      R7;R8;R9;R10]; 
%  s1 = [R1*u(1)*(1-u(1))-D1*u(1); R3; -R3;B_1/epsilon*(u(1)*(1-u(4)/CSF1_max)-u(4));1/epsilon*B_3*(b_EGF*u(3).*u(4)./(K210+K220*drug+u(4))*(1-u(5)/EGF_max)-u(5));1/epsilon*(u(3)*IL4*(1-u(6)/IGF1_max)-u(6))];
s1 = [R1*u(1)*(1-u(1))-D1*u(1); R3; -R3;B_1/epsilon*(u(1)*(1-u(4)/CSF1_max)-u(4));1/epsilon*B_3*(b_EGF*u(3)*(1-u(5)/EGF_max)-u(5));1/epsilon*(u(3)*IL4*(1-u(6)/IGF1_max)-u(6))]; 
%  s2 = [R7;R8;R9;R10]; 
end
% --------------------------------------------------------------
...
u0 = [(1-x1/100)*(0.1+0*0.1.*rand(1,length(x1)))+0.6;x1/100*(0.3+0*0.1.*randn(1,length(x1)))+0.3; x1/100.*(0.3+0*0.1.*randn(1,length(x1)))+0.7; (1-x1/100)*(0.1+0*0.1.*rand(1,length(x1)))+0.3; (1-x1/100)*(0.2+0*0.1.*rand(1,length(x1)))+0.3;0.1;
    0.7;0.72;0.01;0.01];   
% u0 = [(1-x1/100)*(0.2+0*0.1.*rand(1,length(x1)))+0.6;(1-x1/100)*(0.3+0*0.1.*randn(1,length(x1)))+0.4; x1/100.*(0.3+0*0.1.*randn(1,length(x1)))+0.4; (1-x1/100)*(0.3+0*0.01.*rand(1,length(x1)))+0.7; x1/100*(0.3+0*0.01.*rand(1,length(x1)))+0.7;x1/100*(0.3+0*0.01.*rand(1,length(x1)))+0.1;
%     x1/100*(0.2+0*0.1.*rand(1,length(x1)))+0.6; x1/100*(0.1+0*0.1.*rand(1,length(x1)))+0.5; x1/100*(0.01+0*0.1.*rand(1,length(x1)))+0.01;x1/100*(0.01+0*0.1.*rand(1,length(x1)))+0.01 ];                                 
...
% u0 = [value1;value2;value3;value4;value5;value6];
% u0 = [value1;value2;value3;value4;value5;value6;EGFR_0;ERK_0;IGF1R_0;AKT_0];
