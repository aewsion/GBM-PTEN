...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Parameters
if 0==0
    Parameter=[alpha_AKT alpha_ERK B_CSF1 B_CSF1RI B_M1M2 K1 K2 K3 K4 K5 K_CSF1RI K_A d_TM1 r_T B_3 S_IGF1 D_T B_Gal9];%14
else
    Parameter=[alpha_AKT alpha_ERK  B_Gal9 B_M1M2 K1 K2 K3 K4 K5 K_Gal9I d_TM1 r_T];%12
end
...
lb = Parameter*0.5; % lower bounds for Parameters
ub = Parameter*2; % upper bounds for Parameters
...
for i=1:1:p
    ...
    if C(i,timelength)~=min(C(i,:))
        r_RG(i)=(C(i,timelength)-min(C(i,:)))/(timelength-find(C(i,:)==min(C(i,:)),1));%复发
    else
        r_RG(i)=0;
    end
    if C(i,1)~=min(C(i,:))
...
if 0==0
    ...
    D_T_1=D(:,17);
    B_Gal9_1=D(:,18);
    Parameter_1=[alpha_AKT_1 alpha_ERK_1 B_CSF1_1 B_CSF1RI_1 B_M1M2_1 K1_1 K2_1 K3_1 K4_1 K5_1 K_CSF1RI_1 K_A_1 d_TM1_1 r_T_1 B_3_1 S_IGF1_1 D_T_1 B_Gal9_1];%14
    Para_label={'{\alpha}_{AKT}','{\alpha}_{ERK}','B_{CSF1}','B_{CSF1RI}','B_{M1M2}','K1','K2','K3','K4','K5','K_{CSF1RI}','K_A','d_{TM}','r_T','S_A','S_IGF1' 'D_T' ,'B_Gal9'};
else
    % %%% Ranking
    alpha_AKT_1=D(:,1);
    alpha_ERK_1=D(:,2);
    ...
    d_TM1_1=D(:,11);
    r_T_1=D(:,12);
    Parameter_1=[alpha_AKT_1 alpha_ERK_1  B_Gal9_1 B_M1M2_1 K1_1 K2_1 K3_1 K4_1 K5_1 K_Gal9I_1 d_TM1_1 r_T_1];%12
    Para_label={'{\alpha}_{AKT}','{\alpha}_{ERK}','B_{Gal9}','B_{M1M2}','K1','K2','K3','K4','K5','K_Gal9I','d_{TM}','r_T'};
end

r_RG_O=r_RG';%转置
r_K_O=r_K';

[P_R,ind_P]=sort(Parameter_1);%sort对每列排序,参数一排序后矩阵，参数二排序后所对应的原坐标矩阵
...
for j=1:num_para%num_para参数个数
    ...
    [rho,pval] = corr(death_day,Parameter_1(:,j));
    CC(j)=rho;
    Pvalue(j)=pval;
end
...
for j=1:num_para %num_para参数的个数
    a=1:num_para;
    a(a==j)=[];
    size(r_RG_O)
    size(Parameter_1(:,j))
    size(Parameter_1(:,a))
    [rho_RG,pval_RG] = partialcorr(r_RG_O,Parameter_1(:,j),Parameter_1(:,a));
    [rho_K,pval_K] = partialcorr(r_K_O(~isnan(r_K_O)),Parameter_1(~isnan(r_K_O),j),Parameter_1(~isnan(r_K_O),a));
    PCC_RG(j)=rho_RG;
    PCC_K(j)=rho_K;
    Pvalue_RG(j)=pval_RG;
    Pvalue_K(j)=pval_K;
end
save('.\mat\Loss_1.mat','PCC_RG','Pvalue_RG');%(PTEN_Null,LOSS), (PTEN_WT,WT),(CSF1R-I,1),(No treatment,0)
...
set(gca,'xticklabel',Para_label)
rotateticklabel(gca,315);
% xlabel('Parameter_1s','FontWeight','Bold','FontSize',11);
ylabel('Sensitivity Coefficient','FontWeight','Bold','FontSize',11);
...
% xlabel('Parameters','FontWeight','Bold','FontSize',11);
ylabel('Sensitivity Coefficient','FontWeight','Bold','FontSize',11);
title('Partial Correlation Coefficients','FontWeight','Bold','FontSize',11);
set(gca,'FontSize',14);
figure_FontSize=11;
set(get(gca,'XLabel'),'FontSize',figure_FontSize,'Vertical','top');
% set(get(gca,'YLabel'),'FontSize',figure_FontSize,'Vertical','middle');
...
% xlabel('Parameters','FontWeight','Bold','FontSize',11);
ylabel('Sensitivity Coefficient','FontWeight','Bold','FontSize',11);
title('Pvalue','FontWeight','Bold','FontSize',11);
set(gca,'FontSize',14);
figure_FontSize=11;
set(get(gca,'XLabel'),'FontSize',figure_FontSize,'Vertical','top');
% set(get(gca,'YLabel'),'FontSize',figure_FontSize,'Vertical','middle');
