...
while ~done
  ...
  while true                            % Evaluate the formula.
    ...
    [ynew,znew,iter,itfail,rate] = ...
        itsolve(Mtnew,tnew,ynew,znew,gamma,h,L,U,P,Q,R,ode,rtol,...
                wt,rate,Mtype,Mfun,Margs,DAE,RowScale);
  ...
  if output_sol
    ...
    if nout > length(tout)
      ...
      zdata = [zdata, zeros(neq,chunk)]; %#ok<AGROW>
      znewdata = [znewdata, zeros(neq,chunk)]; %#ok<AGROW>
    end
    tout(nout) = tnew; %#ok<AGROW>
    yout(:,nout) = ynew; %#ok<AGROW>
    zdata(:,nout) = z; %#ok<AGROW>
    znewdata(:,nout) = znew; %#ok<AGROW>
  end  
...
function [y,z,iter,itfail,rate] = ...
    itsolve(M,t,y,z,gamma,h,L,U,P,Q,R,ode,rtol,wt,rate,...
            Mtype,Mfun,Margs,DAE,RowScale)
...
% iteration. The function f(t,y) is given by ode(t,y). 
