...
while ~done
  ...
  while true                            % Evaluate the formula.
    ...
    if Mtype == 2   % M(t)
      if odeIsFuncHandle
        ...
        Mt2 = Mfun(t2,y2,Margs{:});
      end
    end

    [y2,z2,iter,itfail1,rate] = ...
        itsolve(Mt2,t2,y2,z2,d,h,L,U,P,Q,R,ode,...
    ...
    if ~itfail1
      ...
      if Mtype == 2   % M(t)
        if odeIsFuncHandle
          Mtnew = Mfun(tnew,Margs{:}); 
        else
          Mtnew = Mfun(tnew,ynew,Margs{:});
        end
      end
      
      [ynew,znew,iter,itfail2,rate] = ...
          itsolve(Mtnew,tnew,ynew,znew,d,h,L,U,P,Q,R,ode,rtol,wt,rate,...
                  Mtype,Mfun,Margs);
      nfevals = nfevals + iter;         
      nsolves = nsolves + iter;                  
    end
  ...
  if output_sol
    ...
    if nout > length(tout)
      ...
      t2data = [t2data, zeros(1,chunk)]; %#ok<AGROW>
      y2data = [y2data, zeros(neq,chunk)]; %#ok<AGROW>
    end
    tout(nout) = tnew; %#ok<AGROW>
    yout(:,nout) = ynew; %#ok<AGROW>
    t2data(nout) = t2; %#ok<AGROW>
    y2data(:,nout) = y2; %#ok<AGROW>
  end  
     ...
     case 'RequestedPoints'    % output only at tspan points
      ...
      while next <= ntspan  
        ...
        if tspan(next) == tnew
          ...
          yout_new = [yout_new, ntrp23tb(tspan(next),t,y,tnew,ynew,t2,y2,idxNonNegative)]; %#ok<AGROW>
        end  
        next = next + 1;
      end
    end
...
function [y,z,iter,itfail,rate] = ...
    itsolve(M,t,y,z,d,h,L,U,P,Q,R,ode,rtol,wt,rate,Mtype,Mfun,Margs)
...
% iteration. The function f(t,y) is given by ode(t,y,odeArgs{:}). 
% Similarly, if M is state-dependent, it is given by Mfun(t,y,Margs{:}). 
