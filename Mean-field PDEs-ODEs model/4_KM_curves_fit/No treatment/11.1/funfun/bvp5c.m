...
while ~done
  ...
  for iter = 1 : maxNewtIter	
    ...
    if needGlobalJacobian
      ...
      dPHIdy = decomposition(colloc_Jac(X,Y,F,BCval));
  ...
  % In a region, compute the ODE Jacobian at points in a mesh interval.          
    if isempty(jac)
      if isempty(Jpropagated)
        [Jn,Joptions.fac,~,nF] = ...
            odenumjac(ode,{x(1),y(:,1),region{:}},f(:,1),Joptions); %#ok<CCAT>
        nODEeval = nODEeval + nF;
      else
        Jn = Jpropagated;          
      end   
      [Jnp1,Joptions.fac,~,nF] = ...
          odenumjac(ode,{x(4),y(:,4),region{:}},f(:,4),Joptions); %#ok<CCAT>
      nODEeval = nODEeval + nF;
      if norm(Jnp1 - Jn,1) <= 0.25*(norm(Jnp1,1) + norm(Jn,1)) 
        ...
        [Jnc1,Joptions.fac,~,nF1] = ...
            odenumjac(ode,{x(2),y(:,2),region{:}},f(:,2),Joptions); %#ok<CCAT>
        [Jnc2,Joptions.fac,~,nF2] = ...
            odenumjac(ode,{x(3),y(:,3),region{:}},f(:,3),Joptions); %#ok<CCAT>
      ...
      if errEst(i) <= rtol
        ...
        if canRemovePoints && ...
          ...
          if predEst < 0.5 * rtol  % replace 3 intervals with 2                  
            ...
            xx(  end+1) = xip3; %#ok<AGROW>
            yy(:,end+1) = yip3; %#ok<AGROW>
            ff(:,end+1) = fip3; %#ok<AGROW>
        ...
        ff =  odeFcn_region(xx,yy,region);        
        xx(  end+1) = xip1; %#ok<AGROW>
        yy(:,end+1) = yip1; %#ok<AGROW>
        ff(:,end+1) = fip1; %#ok<AGROW>
        i = i + 1;
        xidx = xidx + nstages;
      end  
