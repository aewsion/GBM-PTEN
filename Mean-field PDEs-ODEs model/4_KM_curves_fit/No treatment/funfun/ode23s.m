...
[Mtype, M] =  odemass(odeIsFuncHandle,ode,t0,y0,options,varargin);
if Mtype > 0
  if Mtype > 1
...
while ~done
  ...
  if output_sol
    ...
    if nout > length(tout)
      ...
      k1data = [k1data, zeros(neq,chunk)]; %#ok<AGROW>
      k2data = [k2data, zeros(neq,chunk)]; %#ok<AGROW>
    end
    tout(nout) = tnew; %#ok<AGROW>
    yout(:,nout) = ynew; %#ok<AGROW>
    k1data(:,nout) = k1; %#ok<AGROW>
    k2data(:,nout) = k2; %#ok<AGROW>
  end  
     ...
     case 'RequestedPoints'    % output only at tspan points
      ...
      while next <= ntspan  
        ...
        if tspan(next) == tnew
          ...
          yout_new = [yout_new, ntrp23s(tspan(next),t,y,[],[],h,k1,k2)]; %#ok<AGROW>
        end  
        next = next + 1;
      end
    end
