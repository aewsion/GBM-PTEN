...
for person=1:1:p
    if isempty(find(C(person,:)>=threshold,1))%为空进入
        ...
        if i+50<=p12 && max_death_day>=0.77 && (max_death_day-sum(C(person,i+1:i+50))/50)/max_death_day<0.005
            aaaaaa=1
...
if PTEN==0
    ...
    xlim([20 90]);
    xlabel('Time (Days)','FontWeight','Bold','FontSize',10);
    ...
    figure,
    plot(0:20:140,ST2,'o-- ','Linewidth',2.5,'MarkeredgeColor','b','MarkerfaceColor','b');axis([0,140,0,100])
    hold on, 
    plot(0:20:140,PST2,'o-- ','Linewidth',2.5,'MarkeredgeColor','r','MarkerfaceColor','r');axis([0,140,0,100])
    ...
    ylim([0 1]);
    title('CSF1R-I','FontWeight','Bold','FontSize',16,'fontname','Arial');
    box on;

    ST1=[0 0 0 9 25 21 1 0];%20day-90day
...
% filename=['E:\aa文件\project\project2\4\',num2str(6)];
