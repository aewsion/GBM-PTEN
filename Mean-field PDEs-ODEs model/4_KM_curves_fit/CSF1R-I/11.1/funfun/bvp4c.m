...
while ~done       
  ...
  for iter=1:maxNewtIter	
    if needGlobalJacobian
      ...
      [dPHIdy,NF,NBC] = colloc_Jac(n,x,Y,yp,Fmid,ode,bc,Fjac,Joptions,...
  ...
  if maxres < rtol      
    ...
    meshHistory(end+1,:) = [N,maxres]; %#ok<AGROW>
...
if ismbvp
  FcnArgs = {0,ExtraArgs{:}}; %#ok<CCAT> % % pass region idx
else  
  FcnArgs = ExtraArgs;
end
...
for region = 1:nregions
  ...
  if (NN > Nmax)
    ...
    xx = [xx, xxreg(1:NNreg)]; %#ok<AGROW>
    yy = [yy, yyreg(:,1:NNreg)]; %#ok<AGROW>
    if region < nregions    % possible only for multipoint BVPs
      mbcidxnew = [mbcidxnew, NN]; %#ok<AGROW>
    end
  end
end
...
Ridx = [mbcidx, length(x)];
if ismbvp
  FcnArgs = {0,ExtraArgs{:}}; %#ok<CCAT> % Pass the region index to the ODE function.
else  
  FcnArgs = ExtraArgs;
end
...
if npar == 0
  ...
  if isempty(Fjac)  % use numerical approx
    ...
    [Ji,Joptions.fac,~,nFcalls] = ...
        odenumjac(odeFcn,{xreg(1),yreg(:,1),FcnArgs{:}},freg(:,1),Joptions); %#ok<CCAT>
    nfcn = nfcn+nFcalls;    
    nrmJi = norm(Ji,1);
    ...
    for i = 1:nint
      ...
      [Jip1,Joptions.fac,~,nFcalls] = ...
          odenumjac(odeFcn,{xip1,yip1,FcnArgs{:}},Fip1,Joptions); %#ok<CCAT>
      nfcn = nfcn + nFcalls;
      nrmJip1 = norm(Jip1,1);      
      ...
      if norm(Jip1 - Ji,1) <= 0.25*(nrmJi + nrmJip1)
        ...
        [Jip05,Joptions.fac,~,nFcalls] = ...
            odenumjac(odeFcn,{xip05,yip05,FcnArgs{:}},Fip05,Joptions); %#ok<CCAT>
        nfcn = nfcn + nFcalls;
        twiceJip05 = 2*Jip05;
      end      
  ...
  if isempty(Fjac)  % use numerical approx
    ...
    for i = 1:nint
      ...
      [Jip1,Joptions.fac,dFdpar_ip1,dPoptions.fac,nFcalls] = ...
          Fnumjac(odeFcn,{xip1,yip1,FcnArgs{:}},Fip1,Joptions,dPoptions); %#ok<CCAT>
      nfcn = nfcn + nFcalls; 
      nrmJip1 = norm(Jip1,1);
      ...
      if (norm(Jip1 - Ji,1) <= 0.25*(nrmJi + nrmJip1)) && ...
        ...
        [Jip05,Joptions.fac,dFdpar_ip05,dPoptions.fac,nFcalls] = ...
            Fnumjac(odeFcn,{xip05,yip05,FcnArgs{:}},Fip05,Joptions,dPoptions); %#ok<CCAT>
        nfcn = nfcn + nFcalls;
      end
      twiceJip05 = 2*Jip05;   
