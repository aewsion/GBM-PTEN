...
Environment::Environment(double stepSize, std::string folder, int set, double M0RecProb, double TCProl, double k1,
                        double k_CSF1RI, double k_A, double p_tc, double p_d, int pha, int rand,
                        double b_c, double b_ci, double a_erk, double b_g, int pten, double dose_c,
...
void Environment::plot(CellGrids &cg, Diffusibles &diff, int s){
    ...
    // save Gal9_en
    myfile.open(saveDir+"/spatial/"+std::to_string(s)+"/Gal9_en.csv");
    for(int i=0; i<100; ++i){
      myfile << diff.Gal9_en[i][0]/diff.max_CSF1;
      for(int j=1; j<100; ++j){
        myfile << "," << diff.Gal9_en[i][j]/diff.max_CSF1;
      }
      myfile << std::endl;
    }
    myfile.close();
    ...
    // save GSK3b
    myfile.open(saveDir+"/spatial/"+std::to_string(s)+"/GSK3b.csv");
    for(int i=0; i<100; ++i){
        myfile << diff.GSK3b[i][0];
        for(int j=1; j<100; ++j){
            myfile << "," << diff.GSK3b[i][j];
        }
        myfile << std::endl;
    }
    myfile.close();
    ...
    // save IRF1
    myfile.open(saveDir+"/spatial/"+std::to_string(s)+"/IRF1.csv");
    for(int i=0; i<100; ++i){
      myfile << diff.IRF1[i][0];
      for(int j=1; j<100; ++j){
        myfile << "," << diff.IRF1[i][j];
      }
      myfile << std::endl;
    }
    myfile.close();
    ...
    // save Gal9
    myfile.open(saveDir+"/spatial/"+std::to_string(s)+"/Gal9.csv");
    for(int i=0; i<100; ++i){
      myfile << diff.Gal9[i][0];
      for(int j=1; j<100; ++j){
        myfile << "," << diff.Gal9[i][j];
      }
      myfile << std::endl;
    }
    myfile.close();
    ...
    // save CSF1R_I accumulation
    myfile.open(saveDir+"/spatial/"+std::to_string(s)+"/I.csv");
    for(int i=0; i<100; ++i){
      myfile << diff.I[i][0];
      for(int j=1; j<100; ++j){
        myfile << "," << diff.I[i][j];
    ...
    // save AKT_I
    myfile.open(saveDir+"/spatial/"+std::to_string(s)+"/AKT_I.csv");
    for(int i=0; i<100; ++i){
      myfile << diff.AKT_I[i][0];
      for(int j=1; j<100; ++j){
        myfile << "," << diff.AKT_I[i][j];
      }
      myfile << std::endl;
    }
    myfile.close();
    ...
    // save ERK_I
    myfile.open(saveDir+"/spatial/"+std::to_string(s)+"/ERK_I.csv");
    for(int i=0; i<100; ++i){
      myfile << diff.ERK_I[i][0];
      for(int j=1; j<100; ++j){
        myfile << "," << diff.ERK_I[i][j];
      }
      myfile << std::endl;
    }
    myfile.close();
    ...
    // save Gal9_I
    myfile.open(saveDir+"/spatial/"+std::to_string(s)+"/Gal9_I.csv");
    for(int i=0; i<100; ++i){
      myfile << diff.Gal9_I[i][0];
      for(int j=1; j<100; ++j){
        myfile << "," << diff.Gal9_I[i][j];
      }
      myfile << std::endl;
    }
    myfile.close();
    ...
    // save GSK3b_I
    myfile.open(saveDir+"/spatial/"+std::to_string(s)+"/GSK3b_I.csv");
    for(int i=0; i<100; ++i){
      myfile << diff.GSK3b_I[i][0];
      for(int j=1; j<100; ++j){
        myfile << "," << diff.GSK3b_I[i][j];
      }
      myfile << std::endl;
    }
    myfile.close();
    ...
    // save IRF1_I
    myfile.open(saveDir+"/spatial/"+std::to_string(s)+"/IRF1_I.csv");
    for(int i=0; i<100; ++i){
      myfile << diff.IRF1_I[i][0];
      for(int j=1; j<100; ++j){
        myfile << "," << diff.IRF1_I[i][j];
      }
      myfile << std::endl;
    }
    myfile.close();
...
void Environment::save(Diffusibles &diff){
    ...
    // save average concentration sequence of Gal9_en
    myfile.open(saveDir+"/avgGal9_en.csv", std::ios_base::app);
    myfile << Gal9_en_avg[0]/diff.max_Gal9_en;
    for(int i=1; i<Gal9_en_avg.size(); ++i){
	myfile << "," << Gal9_en_avg[i]/diff.max_Gal9_en;
    }
    myfile << std::endl;
    myfile.close();

    // save maximum concentration sequence of Gal9_en
    myfile.open(saveDir+"/maxGal9_en.csv", std::ios_base::app);
    myfile << Gal9_en_max[0]/diff.max_Gal9_en;
    for(int i=1; i<Gal9_en_max.size(); ++i){
	myfile << "," << Gal9_en_max[i]/diff.max_Gal9_en;
    }
    myfile << std::endl;
    myfile.close();
    ...
    myfile.open(saveDir+"/avgCSF1R_I.csv", std::ios_base::app);
    ...
    myfile.open(saveDir+"/maxCSF1R_I.csv", std::ios_base::app);
    ...
    // save average concentration sequence of IGF1R_I
    myfile.open(saveDir+"/avgIGF1R_I.csv", std::ios_base::app);
    myfile << IGF1R_I_avg[0];
    for(int i=1; i<IGF1R_I_avg.size(); ++i){
	myfile << "," << IGF1R_I_avg[i];
    }
    myfile << std::endl;
    myfile.close();

    // save maximum concentration sequence of IGF1R_I
    myfile.open(saveDir+"/maxIGF1R_I.csv", std::ios_base::app);
    myfile << IGF1R_I_max[0];
    for(int i=1; i<IGF1R_I_max.size(); ++i){
	myfile << "," << IGF1R_I_max[i];
    }
    myfile << std::endl;
    myfile.close();

    // save average concentration sequence of EGFR_I
    myfile.open(saveDir+"/avgEGFR_I.csv", std::ios_base::app);
    myfile << EGFR_I_avg[0];
    for(int i=1; i<EGFR_I_avg.size(); ++i){
	myfile << "," << EGFR_I_avg[i];
    }
    myfile << std::endl;
    myfile.close();

    // save maximum concentration sequence of EGFR_I
    myfile.open(saveDir+"/maxEGFR_I.csv", std::ios_base::app);
    myfile << EGFR_I_max[0];
    for(int i=1; i<EGFR_I_max.size(); ++i){
	myfile << "," << EGFR_I_max[i];
    }
    myfile << std::endl;
    myfile.close();

    // save average concentration sequence of AKT_I
    myfile.open(saveDir+"/avgAKT_I.csv", std::ios_base::app);
    myfile << AKT_I_avg[0];
    for(int i=1; i<AKT_I_avg.size(); ++i){
	myfile << "," << AKT_I_avg[i];
    }
    myfile << std::endl;
    myfile.close();

    // save maximum concentration sequence of AKT_I
    myfile.open(saveDir+"/maxAKT_I.csv", std::ios_base::app);
    myfile << AKT_I_max[0];
    for(int i=1; i<AKT_I_max.size(); ++i){
	myfile << "," << AKT_I_max[i];
    }
    myfile << std::endl;
    myfile.close();

    // save average concentration sequence of ERK_I
    myfile.open(saveDir+"/avgERK_I.csv", std::ios_base::app);
    myfile << ERK_I_avg[0];
    for(int i=1; i<ERK_I_avg.size(); ++i){
	myfile << "," << ERK_I_avg[i];
    }
    myfile << std::endl;
    myfile.close();

    // save maximum concentration sequence of ERK_I
    myfile.open(saveDir+"/maxERK_I.csv", std::ios_base::app);
    myfile << ERK_I_max[0];
    for(int i=1; i<ERK_I_max.size(); ++i){
	myfile << "," << ERK_I_max[i];
    }
    myfile << std::endl;
    myfile.close();

    // save average concentration sequence of GSK3b_I
    myfile.open(saveDir+"/avgGSK3b_I.csv", std::ios_base::app);
    myfile << GSK3b_I_avg[0];
    for(int i=1; i<GSK3b_I_avg.size(); ++i){
	myfile << "," << GSK3b_I_avg[i];
    }
    myfile << std::endl;
    myfile.close();

    // save maximum concentration sequence of GSK3b_I
    myfile.open(saveDir+"/maxGSK3b_I.csv", std::ios_base::app);
    myfile << GSK3b_I_max[0];
    for(int i=1; i<GSK3b_I_max.size(); ++i){
	myfile << "," << GSK3b_I_max[i];
    }
    myfile << std::endl;
    myfile.close();

    // save average concentration sequence of IRF1_I
    myfile.open(saveDir+"/avgIRF1_I.csv", std::ios_base::app);
    myfile << IRF1_I_avg[0];
    for(int i=1; i<IRF1_I_avg.size(); ++i){
	myfile << "," << IRF1_I_avg[i];
    }
    myfile << std::endl;
    myfile.close();

    // save maximum concentration sequence of IRF1_I
    myfile.open(saveDir+"/maxIRF1_I.csv", std::ios_base::app);
    myfile << IRF1_I_max[0];
    for(int i=1; i<IRF1_I_max.size(); ++i){
	myfile << "," << IRF1_I_max[i];
    }
    myfile << std::endl;
    myfile.close();

    // save average concentration sequence of Gal9_I
    myfile.open(saveDir+"/avgGal9_I.csv", std::ios_base::app);
    myfile << Gal9_I_avg[0];
    for(int i=1; i<Gal9_I_avg.size(); ++i){
	myfile << "," << Gal9_I_avg[i];
    }
    myfile << std::endl;
    myfile.close();

    // save maximum concentration sequence of Gal9_I
    myfile.open(saveDir+"/maxGal9_I.csv", std::ios_base::app);
    myfile << Gal9_I_max[0];
    for(int i=1; i<Gal9_I_max.size(); ++i){
	myfile << "," << Gal9_I_max[i];
    }
    myfile << std::endl;
    myfile.close();
    ...
    // save average concentration sequence of GSK3b
    myfile.open(saveDir+"/avgGSK3b.csv", std::ios_base::app);
    myfile << GSK3b_avg[0];
    for(int i=1; i<GSK3b_avg.size(); ++i){
	myfile << "," << GSK3b_avg[i];
    }
    myfile << std::endl;
    myfile.close();

    // save maximum concentration sequence of GSK3b
    myfile.open(saveDir+"/maxGSK3b.csv", std::ios_base::app);
    myfile << GSK3b_max[0];
    for(int i=1; i<GSK3b_max.size(); ++i){
	myfile << "," << GSK3b_max[i];
    }
    myfile << std::endl;
    myfile.close();

    // save average concentration sequence of IRF1
    myfile.open(saveDir+"/avgIRF1.csv", std::ios_base::app);
    myfile << IRF1_avg[0];
    for(int i=1; i<IRF1_avg.size(); ++i){
	myfile << "," << IRF1_avg[i];
    }
    myfile << std::endl;
    myfile.close();

    // save maximum concentration sequence of IRF1
    myfile.open(saveDir+"/maxIRF1.csv", std::ios_base::app);
    myfile << IRF1_max[0];
    for(int i=1; i<IRF1_max.size(); ++i){
	myfile << "," << IRF1_max[i];
    }
    myfile << std::endl;
    myfile.close();

    // save average concentration sequence of Gal9
    myfile.open(saveDir+"/avgGal9.csv", std::ios_base::app);
    myfile << Gal9_avg[0];
    for(int i=1; i<Gal9_avg.size(); ++i){
	myfile << "," << Gal9_avg[i];
    }
    myfile << std::endl;
    myfile.close();

    // save maximum concentration sequence of Gal9
    myfile.open(saveDir+"/maxGal9.csv", std::ios_base::app);
    myfile << Gal9_max[0];
    for(int i=1; i<Gal9_max.size(); ++i){
	myfile << "," << Gal9_max[i];
    }
    myfile << std::endl;
    myfile.close();
        ...
        for(int j=1; j<99; j++){
            ...
            if(cg.vas[i][j]==1){
                ...
                if(k<(rec*(M0recProb)*tstep*60*60*(1-m/M_max)) ){  
                    ...
                    cg.allcells[ni][nj] = 1;
                    cg.m0[ni][nj] = 1;
                    mp_list.push_back(Macrophage({ni,nj},1,"M0", K1, K_CSF1RI, K_A, Pha, b_CI, b_C, b_G));
                } 
            }
            ...
            for(int i=1; i<99; i++){
              for(int j=1; j<99; j++){
                if(cg.tc[i][j]+cg.m0[i][j]+cg.m2[i][j]+cg.m1[i][j]+cg.vas[i][j]+cg.dead[i][j]!=cg.allcells[i][j]){
                  std::cout<<"recruitM0 error "<<std::endl;
    ...
    for(int i=49; i<50; i++){
        for(int j=49; j<50; j++){
                ...
                tc_list.push_back(Tumor({i,j}, TCProlTime, z, 1, p_TC, p_D,
                                                EGFRi, IGF1Ri, AKTi, ERKi, GSK3bi, 
                                                IRF1i, Gal9i, a_ERK, TC_max, pTEN));
                z++;
                }  
        }
    }
    ...
    for(int i=35; i<65; i++){
        for(int j=35; j<65; j++){
            ...
            if(cg.allcells[i][j]==0 && rand < 0.02*r){
                ...
                cg.allcells[i][j] = 1;
                if(rand < 0.0*r){
                    mp_list.push_back(Macrophage({i,j}, 1,"M1", K1, K_CSF1RI, K_A, Pha, b_CI, b_C, b_G));
                    cg.m0[i][j] = 0;
                    cg.m1[i][j] = 1;
                }
                else{
                    mp_list.push_back(Macrophage({i,j}, 1,"M2", K1, K_CSF1RI, K_A, Pha, b_CI, b_C, b_G));
                    cg.m0[i][j] = 0;
                    cg.m2[i][j] = 1;
                }
            }
...
void Environment::updateTimeCourses(int s, CellGrids &cg, Diffusibles &diff) {
    ...
    for(int i=1;i<99;i++){
        for(int j=1;j<99;j++){
            ...
            avgGal9_en += diff.Gal9_en[i][j];
            
            avgCSF1R_I += diff.CSF1R_I[i][j];
            avgIGF1R_I += diff.IGF1R_I[i][j];
            avgEGFR_I += diff.EGFR_I[i][j];
            avgAKT_I += diff.AKT_I[i][j];
            avgERK_I += diff.ERK_I[i][j];
            avgGSK3b_I += diff.GSK3b_I[i][j];
            avgIRF1_I += diff.IRF1_I[i][j];
            avgGal9_I += diff.Gal9_I[i][j];
            
            avgAKT += diff.AKT[i][j];
            avgERK += diff.ERK[i][j];
            avgIGF1R += diff.IGF1R[i][j];
            avgEGFR += diff.EGFR[i][j];
            avgGSK3b += diff.GSK3b[i][j];
            avgIRF1 += diff.IRF1[i][j];
            avgGal9 += diff.Gal9[i][j];
            
            if(diff.CSF1[i][j] > maxCSF1){maxCSF1=diff.CSF1[i][j];}
            if(diff.Gal9_en[i][j] > maxGal9_en){maxGal9_en=diff.Gal9_en[i][j];}
            ...
            if(diff.AKT_I[i][j] > maxAKT_I){maxAKT_I=diff.AKT_I[i][j];}
            if(diff.ERK_I[i][j] > maxERK_I){maxERK_I=diff.ERK_I[i][j];}
            if(diff.GSK3b_I[i][j] > maxGSK3b_I){maxGSK3b_I=diff.GSK3b_I[i][j];}
            if(diff.IRF1_I[i][j] > maxIRF1_I){maxIRF1_I=diff.IRF1_I[i][j];}
            if(diff.Gal9_I[i][j] > maxGal9_I){maxGal9_I=diff.Gal9_I[i][j];}
            ...
            if(diff.GSK3b[i][j] > maxGSK3b){maxGSK3b=diff.GSK3b[i][j];}
            if(diff.IRF1[i][j] > maxIRF1){maxIRF1=diff.IRF1[i][j];}
            if(diff.Gal9[i][j] > maxGal9){maxGal9=diff.Gal9[i][j];}
        }
    }
    ...
    avgGal9_en = avgGal9_en/(100*100);
    
    avgCSF1R_I = avgCSF1R_I/(100*100);
    avgIGF1R_I = avgIGF1R_I/(100*100);
    avgEGFR_I = avgEGFR_I/(100*100);
    avgAKT_I = avgAKT_I/(100*100);
    avgERK_I = avgERK_I/(100*100);
    avgGSK3b_I = avgGSK3b_I/(100*100);
    avgIRF1_I = avgIRF1_I/(100*100);
    avgGal9_I = avgGal9_I/(100*100);
    ...
    if(sumc > 0){
        ...
        avgGSK3b = avgGSK3b/sumc;
        avgIRF1 = avgIRF1/sumc;
        avgGal9 = avgGal9/sumc;
    }
    ...
    Gal9_en_max.push_back(maxGal9_en);
    Gal9_en_avg.push_back(avgGal9_en);
    ...
    AKT_I_avg.push_back(avgAKT_I);
    AKT_I_max.push_back(maxAKT_I);
    ERK_I_avg.push_back(avgERK_I);
    ERK_I_max.push_back(maxERK_I);
    GSK3b_I_avg.push_back(avgGSK3b_I);
    GSK3b_I_max.push_back(maxGSK3b_I);
    IRF1_I_avg.push_back(avgIRF1_I);
    IRF1_I_max.push_back(maxIRF1_I);
    Gal9_I_avg.push_back(avgGal9_I);
    Gal9_I_max.push_back(maxGal9_I);
    ...
    GSK3b_avg.push_back(avgGSK3b);
    GSK3b_max.push_back(maxGSK3b);
    IRF1_avg.push_back(avgIRF1);
    IRF1_max.push_back(maxIRF1);
    Gal9_avg.push_back(avgGal9);
    Gal9_max.push_back(maxGal9);
...
void Environment::printStep(Diffusibles &diff) {
    ...
    std::cout
        ...
        << "Avg/Max Gal9_en: " << Gal9_en_avg[s]/diff.max_Gal9_en << " | " << Gal9_en_max[s]/diff.max_Gal9_en << "\n"
        << "Avg/Max CSF1R_I: " << CSF1R_I_avg[s] << " | " << CSF1R_I_max[s] << "\n"
        << "Avg/Max IGF1R_I: " << IGF1R_I_avg[s] << " | " << IGF1R_I_max[s] << "\n"
        << "Avg/Max AKT_I: " << AKT_I_avg[s] << " | " << AKT_I_max[s] << "\n";             
}
...
void Environment::saveall(CellGrids &cg, Diffusibles &diff, int s){
    ...
    // save Gal9_en spatial information
    myfile.open(saveDir+"/saveall/"+std::to_string(s)+"/Gal9_en.csv");
    for(int i=0; i<100; ++i){
      myfile << diff.Gal9_en[i][0]/diff.max_Gal9_en;
      for(int j=1; j<100; ++j){
        myfile << "," << diff.Gal9_en[i][j]/diff.max_Gal9_en;
      }
      myfile << std::endl;
    }
    myfile.close();
...
void Environment::load(){
    ...
    std::string line; 
    std::string address = "./Virtual_TME_input/p_ten_growth_wild";
    if(pTEN == 0){
      address = "./Virtual_TME_input/p_ten_growth_null";
    }
    // load tumor cells
    ...
    // load Gal9_en
    std::ifstream data_G(address + "/Gal9_en.csv");
    while(std::getline(data_G, line)){
      std::istringstream sin(line);      
      std::string tmp5;
      std::vector<double> tmp6;
      while(std::getline(sin, tmp5, ',')){          
      ...
      Gal9_en_matrix.push_back(tmp6);  
    }
    std::cout<<"Gal9_en_matrix loaded"<<std::endl; 
    ...
    // load IGF1R_I
    std::ifstream data_IR_I(address + "/IGF1R_I.csv");
    while(std::getline(data_IR_I, line)){ 
      std::istringstream sin(line);      
      std::string tmp13;
      std::vector<double> tmp14;
      while(std::getline(sin, tmp13, ',')){           
      ...
      IGF1R_I_matrix.push_back(tmp14);  
    }
    std::cout<<"IGF1RI_matrix loaded"<<std::endl; 
 
     // load EGFR_I
     std::ifstream data_ER_I(address + "/EGFR_I.csv");
     while(std::getline(data_ER_I, line)){ 
       std::istringstream sin(line);      
       std::string tmp13;
       std::vector<double> tmp14;
       while(std::getline(sin, tmp13, ',')){           
       ...
       EGFR_I_matrix.push_back(tmp14);  
     }
     std::cout<<"EGFRI_matrix loaded"<<std::endl; 
...
void Environment::initialization(CellGrids &cg, Diffusibles &diff){
    ...
    for(int i=0; i<100; i++){
        for(int j=0; j<100; j++){    
            ...
            diff.Gal9_en[i][j] = Gal9_en_matrix[i][j] * diff.max_Gal9_en;
            diff.EGF[i][j] = EGF_matrix[i][j] * diff.max_EGF;
            diff.IGF1[i][j] = IGF1_matrix[i][j] * diff.max_IGF1;
            diff.CSF1R_I[i][j] = CSF1R_I_matrix[i][j];
            diff.IGF1R_I[i][j] = IGF1R_I_matrix[i][j];
            diff.EGFR_I[i][j] = EGFR_I_matrix[i][j];
            //diff.Gal9_I[i][j] = Gal9_I_matrix[i][j];
            //diff.AKT_I[i][j] = AKT_I_matrix[i][j];
            //diff.ERK_I[i][j] = ERK_I_matrix[i][j];
            //diff.GSK3b_I[i][j] = GSK3b_I_matrix[i][j];
            //diff.IRF1_I[i][j] = IRF1_I_matrix[i][j];
    ...
    for(int i=0; i<tc_matrix.size(); i++){
        ...
        tc_list.push_back(Tumor({std::stoi(tc_matrix[i][0]),std::stoi(tc_matrix[i][1])},
                                ...
                                std::stod(tc_matrix[i][20]),
                                std::stod(tc_matrix[i][21]),
                                std::stod(tc_matrix[i][22]),
                                a_ERK, TC_max, pTEN));
      
        diff.Gal9[std::stoi(tc_matrix[i][0])][std::stoi(tc_matrix[i][1])] = std::stod(tc_matrix[i][22]);
    ...
    for(int i=0; i<mp_matrix.size(); i++){
        ...
        mp_list.push_back(Macrophage({std::stoi(mp_matrix[i][0]),std::stoi(mp_matrix[i][1])},
                                1,mp_matrix[i][2], K1, K_CSF1RI, K_A, Pha, b_CI, b_C, b_G));
...
void Environment::simulate(double days){
    ...
    for(int s=0; s<days*24/tstep; s++){
        ...
        diff.diffusion(cg, tstep, dose_C, dose_I, dose_A);
        ...
        // plot spatial states
        if(fmod(s*tstep, 48 * tstep) == 0){           
            plot(cg, diff, s);
        }
        
        // save virtual TME (if necessary)
        /*
        if(tc_list.size() > 0.6*TC_max && save_time <= 2){
        ...
        */
        ...
        // break condition
        if(tc_num[s] == 0 || tc_num[s] >= 0.9 * TC_max){
...
/*
    ...
    X. Ni, W. Wu, X. Sun, J. Ma, Z. Yu, X. He, J. Cheng, P. Xu, H. Liu, T. Shang, S. Xi, 
    J. Wang, J. Zhang, Z. Chen, Interrogating glioma-M2 macrophage interactions identifies 
    Gal-9/Tim-3 as a viable target against PTEN-null glioblastoma. Sci Adv 8, eabl5165 (2022).
    ...
    H. Lin, J. Zhang, Q. Nie, X. Sun, Multiscale Modeling of Tumor-Macrophage Interactions 
    Underlying Immunotherapy Resistance in Glioblastoma. 
    Multiscale Modeling \& Simulation 23, 838-863 (2025).
