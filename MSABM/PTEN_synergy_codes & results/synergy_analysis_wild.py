...
def R12_Calculate(group_name, set_list, dose_C_list, dose_A_list, dose_C_index, dose_A_index): 
    output_directory = f"./results/figures/synergy_analysis_wild/{group_name}/"
    ...
    for n in set_list:
        ...
        if os.path.exists(file_path):
            ...
            data_list = [row + [0] * (max_cols - len(row)) for row in data_list]
...
def Synergy_Index_Calculate(group_name, R12, R1, R2, dose1_name, dose2_name):
    output_directory = f"./results/figures/synergy_analysis_wild/{group_name}_index/"
    os.makedirs(output_directory, exist_ok=True)  # Create directory if it doesn't exist    
...
def main():
    ...
    dose_list = list(np.round(np.arange(0, 2.1, 0.1), 1))
    ...
    Synergy_Index_Calculate(group_name, R_CA, R_C, R_A, 'IGF1R_I', 'AKT_I')
