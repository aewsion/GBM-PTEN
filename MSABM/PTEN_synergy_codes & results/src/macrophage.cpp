...
Macrophage::Macrophage(std::array<int, 2> loc, int initial, std::string State, double k1, 
                        double k_CSF1RI, double k_A, int pha, double b_ci, double b_c, double b_g){
    // representation, unit, references of parameters
    // are listed in Supplementary Table 'Macrophages' part    
    state = State;
    location = loc;
...
void Macrophage::migration(CellGrids &cg, Diffusibles &diff){
    ...
    for(int q=0;q<5;q++){
        ...
        G[q]=diff.Gal9_en[i+ix[q]][j+jx[q]];
    }
    std::mt19937 g(rd());
    std::uniform_real_distribution<> Dis(0.0,1.0);

    double h = diff.dx; // 15um = grid size
    double k = 2250*0.22; // choose a suitable k

    // calculate probability of latent migration choices
    double probs[5]={0,0,0,0,0};
    probs[4] = 1 - 4*k*D_M/pow(h,2) - k*a_MC*(C[0]+C[1]+C[2]+C[3]-4*C[4])/pow(h,2) - k*a_MG*(G[0]+G[1]+G[2]+G[3]-4*G[4])/pow(h,2); // remain stationary
    probs[0] = (1 - cg.allcells[i+ix[0]][j+jx[0]]) * (k*D_M/pow(h,2) - k*a_MC*(C[2]-C[0])/(4*pow(h,2)) - k*a_MG*(G[2]-G[0])/(4*pow(h,2))); // move up
    probs[1] = (1 - cg.allcells[i+ix[1]][j+jx[1]]) * (k*D_M/pow(h,2) - k*a_MC*(C[3]-C[1])/(4*pow(h,2)) - k*a_MG*(G[3]-G[1])/(4*pow(h,2))); // move right
    probs[2] = (1 - cg.allcells[i+ix[2]][j+jx[2]]) * (k*D_M/pow(h,2) - k*a_MC*(C[0]-C[2])/(4*pow(h,2)) - k*a_MG*(G[0]-G[2])/(4*pow(h,2))); // move down
    probs[3] = (1 - cg.allcells[i+ix[3]][j+jx[3]]) * (k*D_M/pow(h,2) - k*a_MC*(C[1]-C[3])/(4*pow(h,2)) - k*a_MG*(G[1]-G[3])/(4*pow(h,2))); // move left
...
int Macrophage::transformation(CellGrids &cg, Diffusibles &diff){
    ...
    double G_en = diff.Gal9_en[i][j]/diff.max_Gal9_en;
    double C_I = diff.CSF1R_I[i][j]; // drug dose
    double G_I = diff.Gal9_I[i][j];

    double H1 = C/(K1 + K_CSF1RI*C_I + C);
    double H2 = G_en/(K2 + K_Gal9I*G_I + G_en);
    double H3 = diff.I[i][j]/(K_A + diff.A_0 + diff.I[i][j]);
...
void Macrophage::simulaion(double tstep, CellGrids &cg, Diffusibles &diff, double depletion){
    ...
    // simualte phagocytosis of M1
    for(int i=0; i<Pha; i++){
        if(diff.time > 0*24 && i<Pha-1 && state == "M1" 
               && now_phagocytose < max_phagocytose){
