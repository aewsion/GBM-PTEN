...
void Diffusibles::diffusion(CellGrids &cg, double tstep, double dose_C) {
    ...
    for (int q = 0; q < t; q++) {
        for(int i=1; i<99; ++i){
            for(int j=1; j<99; ++j){
                if(cg.vas[i][j] == 1){
                    IGF1R_I[i][j] = IGF1R_I[i][j] + q_IGF1R_I * (0.0 - IGF1R_I[i][j]);
                    EGFR_I[i][j] = EGFR_I[i][j] + q_EGFR_I * (0.0 - EGFR_I[i][j]);
                }      
            }
        }
        maxDif = 0;
        for (int i = 1; i < 99; i++) {
            for (int j = 1; j < 99; j++) {
                ...
                Gal9_I[i][j] < 0 || AKT_I[i][j] < 0 || ERK_I[i][j] < 0 || GSK3b_I[i][j] < 0 || IRF1_I[i][j] < 0){
                    std::cout << "IGF1 " << IGF1[i][j] << std::endl;
                    std::cout << "CSF1R_I "<< CSF1R_I[i][j] << std::endl;
                    std::cout << "Gal9_I "<< Gal9_I[i][j] << std::endl;
                    std::cout << "IGF1R_I "<< IGF1R_I[i][j] << std::endl;
                    std::cout << "EGFR_I "<< EGFR_I[i][j] << std::endl;
                    std::cout << "AKT_I "<< AKT_I[i][j] << std::endl;
                    std::cout << "ERK_I "<< ERK_I[i][j] << std::endl;
                    std::cout << "GSK3b_I "<< GSK3b_I[i][j] << std::endl;
                    std::cout << "IRF1_I "<< IRF1_I[i][j] << std::endl;
