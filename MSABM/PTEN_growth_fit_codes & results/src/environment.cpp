...
Environment::Environment(double stepSize, std::string folder, int set, double M0RecProb, double TCProl, double k1,
                        ...
                        double b_c, double b_ci, double a_erk, double b_g, int pten){   
...
void Environment::load(){
    ...
    std::string line; 
    std::string address = "Your_virtual_TME_path";
    // load tumor cells
...
void Environment::simulate(double days){
    ...
    for(int s=0; s<days*24/tstep; s++){
        ...
        if(tc_list.size() > 0.45*TC_max && save_time <= 2){
            std::cout<<"saveall"<<std::endl;
            saveall(cg, diff, s);
            save_time++;
        }
        
        if(tc_list.size() > 0.5*TC_max && save_time <= 4){
            std::cout<<"saveall"<<std::endl;
            saveall(cg, diff, s);
            save_time++;
        }
        
        if(tc_list.size() > 0.55*TC_max && save_time <= 6){
            std::cout<<"saveall"<<std::endl;
            saveall(cg, diff, s);
            save_time++;
        }
        if(tc_list.size() > 0.6*TC_max && save_time <= 8){
            std::cout<<"saveall"<<std::endl;
            saveall(cg, diff, s);
            save_time++;
        }
        if(tc_list.size() > 0.65*TC_max && save_time <= 10){
            std::cout<<"saveall"<<std::endl;
            saveall(cg, diff, s);
            save_time++;
        }
