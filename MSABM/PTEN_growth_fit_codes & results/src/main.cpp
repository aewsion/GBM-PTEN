...
int main(int argc, char **argv){
    ...
    double K_CSF1RI = 7.0; // Michaelis constant for the Hill function of CSF1R_I
    ...
    double p2 = Dis_n(g);
    double p3 = Dis_n(g); 
    double b_C, b_CI, a_ERK;
    double b_G = 1.75/12.69;

    if(rand == 0){
        b_C = 0.675;//4.69/12.69; // adjustment coefficient of Hill function H_C
        b_CI = 0.45;//6.25/12.69; // adjustment coefficient of Hill function H_I
        a_ERK = 7.15; // coefficient associated with ERK to promote tumor growth
    }
    else if(rand == 1){
        b_C = 0.675*(1.0+0.1*p1);
        b_CI = 0.45*(1+0.1*p2);
        a_ERK = 7.15*(1+0.1*p3);        
    }
    if(b_CI < 0){
